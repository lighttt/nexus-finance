...
const aiRingCircumference = 339.29
const aiRingOffset = computed(() => aiRingCircumference - (Math.max(0, Math.min(100, aiRiskPercent.value)) / 100) * aiRingCircumference)
const aiDriverLabel = computed(() => props.intelligence.aiInsight?.sentimentCorrelation.marketDriver.replace(/-/g, ' ') ?? 'AI unavailable')
...
const insightCards = computed(() => {
  ...
  return [
    ...
    },
    {
      ...
      toneLabel: `Downside protection ${props.intelligence.aiInsight.volatilityStress.downsideProtection}`,
...
<template>
  <article
    class="nf-ai-panel overflow-hidden rounded-[28px] border bg-[linear-gradient(135deg,rgba(255,249,237,0.98),rgba(241,246,231,0.92)_58%,rgba(232,241,249,0.85))] p-6 shadow-sm"
    :class="props.intelligence.aiInsight ? aiRiskLevelClass : 'border-[var(--nf-line)]'"
  >
    <div class="nf-ai-shell space-y-5">
      <div class="min-w-0">
        <div class="flex flex-wrap items-start justify-between gap-3">
          <div>
            <p class="text-xs font-semibold uppercase tracking-[0.16em] text-[var(--nf-muted)]">AI Insight</p>
            <h2 class="mt-2 text-2xl font-semibold tracking-tight">Risk outlook</h2>
          </div>
          <span class="rounded-full border border-white/70 bg-white/80 px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--nf-muted)] shadow-sm">
            {{ props.intelligence.aiInsight ? 'Live analysis' : 'Standby' }}
          </span>
        </div>

        <div v-if="props.intelligence.aiInsight" class="mt-6 space-y-5">
          <div class="nf-ai-card rounded-[24px] p-5 shadow-[inset_0_1px_0_rgba(255,255,255,0.65)]">
            <div class="grid gap-5 lg:grid-cols-[320px_minmax(0,1fr)] lg:items-center">
              <div class="mx-auto flex w-full max-w-[320px] flex-col items-center text-center">
                <div class="relative h-[220px] w-[220px]">
                  <svg viewBox="0 0 220 220" class="h-full w-full -rotate-90">
                    <defs>
                  ...
                  <div class="absolute inset-x-0 top-[90px] flex items-end justify-center gap-1">
                    <div class="flex items-end justify-center gap-1">
                      ...
                      <span class="pb-3 text-sm font-semibold text-[var(--nf-muted)]">/100</span>
                    </div>
                  </div>
                </div>

                <p class="max-w-[250px] text-center text-sm leading-6 text-[var(--nf-muted)]">
                  Measures how far recent price action is moving away from the news tone.
                </p>
              </div>

              <div class="w-full space-y-3">
                <div class="grid grid-cols-1 gap-3 sm:grid-cols-2">
                  <div class="nf-ai-card-soft rounded-2xl p-4 text-left">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--nf-muted)]">Primary driver</p>
                  ...
                  <div class="nf-ai-card-soft rounded-2xl p-4 text-left">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--nf-muted)]">Confidence</p>
                    <div class="mt-2 flex items-center gap-2">
                ...
                <div class="nf-ai-card-soft overflow-hidden rounded-2xl">
                  ...
                  <div class="flex items-center justify-between px-4 py-3 text-xs font-semibold uppercase tracking-[0.12em] text-[var(--nf-muted)]">
                    <span>Stable</span>
                    <span>Elevated</span>
                    <span>Stress</span>
                  </div>
                ...
                <div class="rounded-2xl border p-4 text-left" :class="scoreToneClass(props.intelligence.aiInsight.sentimentCorrelation.divergenceRisk)">
                  <p class="text-[11px] font-semibold uppercase tracking-[0.14em]">Current read</p>
                  <p class="mt-2 text-sm font-semibold">{{ props.intelligence.aiInsight.sentimentCorrelation.divergenceRisk }} divergence risk</p>
                </div>
              </div>
            </div>
          </div>

          <div class="nf-ai-card rounded-[24px] p-5">
            <div class="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div>
                <p class="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--nf-muted)]">AI summary</p>
                <h3 class="mt-2 text-xl font-semibold">{{ aiSummaryHeading }}</h3>
              </div>
            ...
            <p class="nf-ai-summary-copy mt-4 text-sm leading-6 text-[var(--nf-muted)]">{{ props.intelligence.aiInsight.sentimentCorrelation.summary }}</p>
          </div>
        </div>
        ...
        <div v-else class="mt-6 rounded-[24px] border border-white/70 bg-white/55 p-5 text-sm text-[var(--nf-muted)] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] backdrop-blur-sm">
          Bedrock insight is unavailable. Set `AWS_REGION` and `AWS_BEARER_TOKEN_BEDROCK` to enable AI analysis.
        </div>
      </div>
      ...
      <div class="min-w-0">
        <div v-if="props.intelligence.aiInsight" class="space-y-4">
          <div class="nf-ai-compare-grid">
            <div
              ...
              :key="card.key"
              class="nf-ai-compare-card rounded-[24px] p-5"
            >
              <div class="flex items-start gap-3">
                <span class="inline-flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl text-sm font-bold" :class="card.badgeClass">
                  {{ card.badge }}
                </span>
                <div class="min-w-0">
                  <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--nf-muted)]">{{ card.title }}</p>
                  <p class="mt-2 text-lg font-semibold">{{ card.metric }}</p>
                  <p v-if="card.sublabel" class="mt-1 text-sm font-medium capitalize">{{ card.sublabel }}</p>
                  <span
                    v-if="card.toneLabel"
                    class="mt-2 inline-flex rounded-full border px-2.5 py-1 text-[11px] font-semibold uppercase tracking-[0.1em]"
              ...
              <div v-if="card.statTiles.length" class="nf-ai-stat-grid mt-4 text-sm">
                <div
                  v-for="tile in card.statTiles"
                  :key="tile.label"
                  class="nf-ai-stat-tile rounded-xl p-3"
                >
              ...
              <p class="mt-4 text-sm leading-6 text-[var(--nf-muted)]">{{ card.summary }}</p>
            </div>
          </div>

          <div class="nf-ai-card rounded-[24px] p-5">
            <p class="text-xs font-semibold uppercase tracking-[0.14em] text-[var(--nf-muted)]">Evidence Trail</p>
            <div class="mt-4 grid gap-3 md:grid-cols-2">
              <div
                v-for="(item, index) in props.intelligence.aiInsight.signals"
                :key="item"
                class="nf-ai-evidence-card group rounded-2xl px-4 py-4"
              >
                <div class="flex gap-3 pl-2">
                  ...
                  <div class="min-w-0">
                    <p class="text-[11px] font-semibold uppercase tracking-[0.14em] text-[var(--nf-muted)]">Signal {{ index + 1 }}</p>
        ...
        <div v-else class="nf-ai-card rounded-[24px] p-5 text-sm text-[var(--nf-muted)]">
          The rest of the symbol intelligence view is still available. Once Bedrock credentials are present, this panel will render a risk gauge, narrative summary, and evidence trail from the AI analysis.
        </div>
      </div>
    </div>
  </article>
